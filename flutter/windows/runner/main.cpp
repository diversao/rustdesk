...
typedef char** (*FUNC_OABRemoteDesk_CORE_MAIN)(int*);
typedef void (*FUNC_OABRemoteDesk_FREE_ARGS)( char**, int);
const char* uniLinksPrefix = "OABRemoteDesk://";
...
{
  HINSTANCE hInstance = LoadLibraryA("libOABRemoteDesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libOABRemoteDesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_OABRemoteDesk_CORE_MAIN OABRemoteDesk_core_main =
      (FUNC_OABRemoteDesk_CORE_MAIN)GetProcAddress(hInstance, "OABRemoteDesk_core_main_args");
  if (!OABRemoteDesk_core_main)
  {
    std::cout << "Failed to get OABRemoteDesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_OABRemoteDesk_FREE_ARGS free_c_args =
      (FUNC_OABRemoteDesk_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = OABRemoteDesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "OABRemoteDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  ...
  // Uri links dispatch
  HWND hwnd = ::FindWindow(_T("FLUTTER_RUNNER_WIN32_WINDOW"), _T("OABRemoteDesk"));
  if (hwnd != NULL) {
  ...
  if (!window.CreateAndShow(
          is_cm_page ? L"OABRemoteDesk - Connection Manager" : L"OABRemoteDesk", origin,
          size, !is_cm_page)) {
      return EXIT_FAILURE;
  }
