...
mod test {
    ...
    fn test_filename_license_string() {
        assert!(get_license_from_string("OABRemoteDesk.exe").is_err());
        assert!(get_license_from_string("OABRemoteDesk").is_err());
        assert_eq!(
            get_license_from_string("OABRemoteDesk-host=server.example.net.exe").unwrap(),
            License {
        ...
        assert_eq!(
            get_license_from_string(
                "OABRemoteDesk-host=server.example.net,api=abc,key=Zm9vYmFyLiwyCg==.exe"
            )
            .unwrap(),
            License {
                host: "server.example.net".to_owned(),
        ...
        assert_eq!(
            get_license_from_string("OABRemoteDesk-host=server.example.net,key=Zm9vYmFyLiwyCg==,.exe")
                .unwrap(),
            License {
                host: "server.example.net".to_owned(),
                key: "Zm9vYmFyLiwyCg==".to_owned(),
