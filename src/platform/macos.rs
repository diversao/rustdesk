...
// remove just one app from all the permissions: tccutil reset All com.carriez.OABRemoteDesk
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/OABRemoteDesk.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another OABRemoteDesk.app
...
pub fn handle_application_should_open_untitled_file() {
    ...
    if x == "--server" || x == "--cm" || x == "--tray" {
        if crate::platform::macos::check_main_window() {
            allow_err!(crate::ipc::send_url_scheme("OABRemoteDesk:".into()));
        }
    }
}
