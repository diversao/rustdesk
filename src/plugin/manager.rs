...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    vec![PluginSource {
        name: "OABRemoteDesk".to_string(),
        url: "https://raw.githubusercontent.com/fufesou/OABRemoteDesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "OABRemoteDesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "OABRemoteDesk wants to uninstall the plugin",
    )
}
